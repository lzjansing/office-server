...
function uploadImageAJAX(_callback) {
    ...
    if(success===true) {
        $.ajaxFileUpload({
            url: ctx + "/upload/img",
            secureuri: false,
            fileElementId: 'uploadImageInput',
            dataType : 'text',
            complete: function() {
                clearFileInput(file);
            },
            success : function(msg) {
                msg = JSON.parse(msg);
                //console.log('path='+msg.param.returnPath);
                if(msg.param.state){
                    //console.log("上传图片成功!地址为："+msg.returnPath);
                    _callback(msg.param.returnPath);
                }else{
                    console.log("上传图片失败!");
                }
            },
        ...
        if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            ICommon.page_loading(msg.join("<br/>"));
        }else {
            page_loading(msg.join("<br/>"), 1500);
        }
        //重置
        success = true;
        msg.length = 0;
...
function clearFileInput(file){
    if ($(file).html()) {  // for IE, Opera, Safari, Chrome
        $(file).html($(file).html());
    } else { // FF(包括3.5)
        $(file).val("");
    }
